...
program inference
    ...
    type(torch_tensor), dimension(3) :: out_tensors
    ...
    ! Layer 2 (4096 x 4096)
    call torch_tensor_from_array(out_tensors(2), out_1, tensor_layout, torch_kCPU)
    call torch_tensor_from_array(out_tensors(2), out_2, tensor_layout, torch_kCPU)

    ! Layer 3 (4096 x 560)
    call torch_tensor_from_array(out_tensors(3), out_2, tensor_layout, torch_kCPU)
    call torch_tensor_from_array(out_tensors(3), out_3, tensor_layout, torch_kCPU)
    ...
    call torch_tensor_delete(out_tensors(3))
